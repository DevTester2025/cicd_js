...
@Component({
  selector: "add-edit-tasktemplete-assets",
  templateUrl:
    "./addedittasktemplateAssets.component.html",
  styles: [
    `
      #assetdetail td,
      #assetdetail th {
...
export class addEditTaskTemplateAssetsComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.appScreens) {
      this.isapproveryn = this.userstoragedata.isapproveryn == "1" ? true : false;
  ...
  displayUserDetails(resourceDetails: any[], r: any, value) {
    console.log(r);
    let retrunStr = value || "-";
    // let udetails=_.find(resourceDetails,(d:any)=>{
    //   return d.fieldname == r.fieldname
    // });
    // if(udetails){
    //   retrunStr = udetails.fieldvalue;
    // }
    if (r.fieldname == this.workpack_defaultAttr[0].fieldname || r.fieldname == this.workpack_defaultAttr[4].fieldname) {
      let user = _.find(this.usersList, (us: any) => {
        return us.userid == value;
      });
      if (user) {
        retrunStr = user.fullname
      }
    }
    return retrunStr;
  }
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.selectedTaskEdit) {
      console.log(this.selectedTaskEdit);
    }
  }
  getTagDetails(tags) {
    let reqObj = {
  ...
  checkAssets() {
    this.selectedResource.forEach((r) => {
      if (this.resourceDetails.details) {
        let key = r.fieldkey;
        if (this.resourceDetails.details.length > 0) {
          this.resourceId = this.resourceDetails.details[0]["resourceid"];
          this.assignWorkflow();
        }
  ...
  startEdit(field, event: MouseEvent): void {
    if ((this.isapproveryn || this.edit) && field.selectedId.readonly != 1) {
      event.preventDefault();
      event.stopPropagation();
  ...
  updateRecord(fieldkey, updatedData) {
    ...
    if (detailObj != undefined && detailObj.id) {
      let reqObj = {
        ...
        resourceid: detailObj.resourceid
      } as any;
      ...
      if (this.selectedField.fieldtype == "Date") {
        let dvalue = moment(reqObj.fieldvalue).format("DD/MMM/YY");
        reqObj.fieldvalue = (dvalue != "Invalid Date" && dvalue != "Invalid date") ? dvalue : "";
      }
      if (this.selectedField.fieldtype == "DateTime") {
        let dvalue = moment(reqObj.fieldvalue).format("DD/MMM/YY h:mm A");
        reqObj.fieldvalue = (dvalue != "Invalid Date" && dvalue != "Invalid date") ? dvalue : "";
      }
      this.assetRecordService.updateDetail(reqObj, "skipApproval=true").subscribe((res) => {
        const response = JSON.parse(res._body);
        if (response.status) {
          this.message.success(response.message);
      ...
      if (this.selectedField.fieldtype == "DOCUMENT") {
        reqObj.fieldvalue = "";
      }
    ...
    _.map(this.resourceData, (item) => {
      if (item.fieldvalue != undefined) {
        ...
        if (item.fieldtype == "Date") {
          let dvalue = moment(obj.fieldvalue).format("DD/MMM/YY");
          obj.fieldvalue = (dvalue != "Invalid Date" && dvalue != "Invalid date") ? dvalue : "";
        }
        if (item.fieldtype == "DateTime") {
          let dvalue = moment(obj.fieldvalue).format("DD/MMM/YY h:mm A");
          obj.fieldvalue = (dvalue != "Invalid Date" && dvalue != "Invalid date") ? dvalue : "";
        }
